...
func TestHarness_CoverageInFooter(t *testing.T) {
	...
	out, errOut := &bytes.Buffer{}, &bytes.Buffer{}
	rep := report.NewCLI(out, errOut)
	run.RunHarness(run.Config{CorpusDir: dir, Probes: 1}, exec, loadStores(t, dir), rep, errOut)
...
func TestHarness_StateInitializer_Supported(t *testing.T) {
	...
	out, errOut := &bytes.Buffer{}, &bytes.Buffer{}
	rep := report.NewCLI(out, errOut)
	code := run.RunHarness(run.Config{CorpusDir: "snap", Probes: 1}, exec, []compare.CorpusStore{store}, rep, errOut)
...
func TestHarness_StateInitializer_NotSupported(t *testing.T) {
	...
	out, errOut := &bytes.Buffer{}, &bytes.Buffer{}
	rep := report.NewCLI(out, errOut)
	run.RunHarness(run.Config{CorpusDir: "snap", Probes: 1}, exec, []compare.CorpusStore{store}, rep, errOut)
